...
class Game:
    ...
    def __init__(self):
        ...
        self.menu_manager = MenuManager(self.display)
        self.running = False
    ...
    def _game_loop(self):
        ...
        while self.state:
            ...
            if choice == 0:
                ...
                self.menu_manager.show_team_menu(self.state)
            elif choice == 2:
                self.menu_manager.show_items_menu(self.state)
            elif choice == 3:
                self.menu_manager.show_badges(self.state)
            elif choice == 4:
                self.menu_manager.show_pokedex(self.state)
            elif choice == 5:
                self._show_trading_menu()
            elif choice == 6:
                self._show_breeding_menu()
            elif choice == 7:
                self.menu_manager.show_type_chart_menu()
            elif choice == 8:
                self.menu_manager.show_sprite_viewer_menu(self.state)
            elif choice == 9:
                self.menu_manager.show_settings_menu()
            elif choice == 10:
                self.save_manager.save_game(self.state)
                print("\nGame saved!")
                input("Press Enter to continue...")
    ...
    def _interact_with_npc(self, npc: NPC):
        ...
        if npc.is_shopkeeper:
            ...
            if choice == 'y':
                self.menu_manager.show_shop_menu(self.state, npc)
        elif npc.is_healer:
            ...
            choice = input("> ").strip().lower()
            if choice == 'y':
                self.menu_manager.show_move_relearner_menu(self.state)
        elif npc.is_trainer:
